...
usage() {
  cat <<HELP
Usage:
  install.sh [latest|vX.Y.Z|X.Y.Z] [install options]
...
Install and upgrade options are passed to scripts/install-local.sh:
  --type gtrace|otlp
...
if [[ ! -f "$TMP_DIR/repo/scripts/install-local.sh" ]]; then
  echo "Invalid archive: scripts/install-local.sh not found" >&2
  exit 1
fi
